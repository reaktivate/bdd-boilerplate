...
class JSM implements IFsm {
  ...
  private transit(transition: string, from: string, to: string | undefined, args?: any | any[]) {
    ...
    return this.observeEvents(
      [
        ...
        changed
          ? this.observersForEvent(lifecycle.onLeave[from] /* onLeave[FromState] */)
          : UNOBSERVED,
   ...
   * @private
   */
  private endTransit(result: boolean): boolean {
    this.pending = false;
    return result;
  }
   ...
   * @private
   */
  private failTransit(result): Error {
    this.pending = false;
    throw result;
  }

  private doTransit(lifecycle) {
   ...
   * @private
   */
  private observersForEvent(event: string): EventItem {
  ...
  private observeEvents(
    eventQueue: EventObserverQueue,
    args,
    previousEvent?: string | null | undefined,
    previousResult?: undefined | boolean
  ) {
    // args : [LifecycleContext, ...props]
    ...
    if (observers.length == 0) {
      ...
      if (eventName) {
        ...
        if (result && typeof result.then === 'function') {
          // result is a Promise
          return result
            .then(this.observeEvents.bind(this, eventQueue, args, eventName))
