 ...
 */
function build<T>(
  cstor?: GenericConstructor | Record<string, unknown> | undefined,
  initOptions?: Options
): StateMachineClass<T & IStateMachine> {
  class EmptyClass {}
  ...
  return class DerivedStateMachine extends chosenBaseClass implements IStateMachine {
    ...
    public fire(transition: string, args?: any) {
      console.log('fire', transition, args);
      return this._fsm.fire(transition, args || []);
    }

    public is(this: IStateMachine, state: State | State[]) {
      return this._fsm.is(state);
    }
    public can(this: IStateMachine, transition: string) {
      return this._fsm.can(transition);
    }
    public cannot(this: IStateMachine, transition: string) {
      return this._fsm.cannot(transition);
    }
    // eslint-disable-next-line prefer-rest-params
    public observe(this: IStateMachine) {
      // eslint-disable-next-line prefer-rest-params
      return this._fsm.observe(arguments);
    }
    public transitions(this: IStateMachine) {
      return this._fsm.transitions();
    }
    public allTransitions(this: IStateMachine) {
      return this._fsm.allTransitions();
    }
    public allStates(this: IStateMachine) {
      return this._fsm.allStates();
    }
    public onInvalidTransition(this: IStateMachine, t, from, to) {
      return this._fsm.onInvalidTransition(t, from, to);
    }
    public onPendingTransition(this: IStateMachine, t, from, to) {
